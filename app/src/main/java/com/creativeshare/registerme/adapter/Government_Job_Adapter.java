...
import com.creativeshare.registerme.activities_fragments.activities.home_activity.fragments.fragment_home.fragment_jobs.Fragment_Government_Sector;
...
public class Government_Job_Adapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private final int ITEM_DATA = 1;
    private final int ITEM_LOAD = 2;

    private List<AllInFo_Model.Data.Gcompanies> data;
    private Context context;
    private Home_Activity activity;
    private Fragment fragment;
    private String current_lang;
    private int i=-1;
    private Fragment_Government_Sector fragment_government_sector;

    public Government_Job_Adapter(List<AllInFo_Model.Data.Gcompanies> data, Context context, Fragment fragment) {
    ...
    public void onBindViewHolder(@NonNull final RecyclerView.ViewHolder holder, int position) {
        ...
        if (holder instanceof MyHolder) {
            ...
            AllInFo_Model.Data.Gcompanies data1 = data.get(position);

            Picasso.with(context).load(Uri.parse(Tags.IMAGE_URL+data1.getLogo())).fit().into(myHolder.imageView);
            myHolder.itemView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if(fragment instanceof Fragment_Government_Sector){
                        fragment_government_sector=(Fragment_Government_Sector)fragment;
                        fragment_government_sector.setid(data.get(myHolder.getLayoutPosition()).getId());
                    }
                    i=position;
